...
class Useful
{
	...
	public static inline function normalise(x : Float, y : Float)
	{
		var norm = Math.sqrt(sqr(x) + sqr(y));
		return ({ x : x/norm, y : y/norm, originalNorm : norm });
	}

	public static var ORIGIN(default, null) : Point = new Point(0, 0);
}